...
from uwcs_website.events.models import Event, Location, val_users, Signup
from uwcs_website.events.models import EventSignup, SeatingRevision
from uwcs_website.memberinfo.models import warwick_week_for, Term
from uwcs_website.shortcuts import begin_week, path_processor
from uwcs_website.memberinfo.models import MemberJoin
from uwcs_website.events.similarity import closest_person
...
class Week:
    ...
    def __str__(self):
        return self.begin.strftime(settings.WEEK_FORMAT_STRING)+" - "+self.end.strftime(settings.WEEK_FORMAT_STRING)
...
def details(request,event_id):
    ...
    try:
        ...
        dict.update({
            'open': s.open.strftime(settings.DATE_FORMAT_STRING),
            'close': s.close.strftime(settings.DATE_FORMAT_STRING),
            'fresher': s.fresher_open.strftime(settings.DATE_FORMAT_STRING),
            'guest': s.guest_open.strftime(settings.DATE_FORMAT_STRING),
