...
from uwcs.website.memberinfo.forms import *
from uwcs.website.shortcuts import * 
...
def index(request):
    ...
    extra_quota = sum([q*s for (q,s) in Quota.objects.filter(user=u,status='PR').values_list('quantity','quota_size')])
...
def lists(request):
    ...
    try:
        from uwcs.website.memberinfo.mailman import *
        try:
